...
Page({
    ...
    getCanvas() {
        ...
        this.count1();
        this.painEye(this.data.canvasBg,'../../images/img/eye.png',this.data.qrLength,this.data.unit);
    },
    // 绘制艺术二维码
    ...
    // 绘制艺术二维码
    painEye(ctx,url,datalistlength,unit) {
        //浏览器加载图片完毕后再绘制图片
        var eye=url;
        ctx.drawImage(eye, 0, 0, 7 * unit, 7 * unit);
        ctx.drawImage(eye, (datalistlength - 7) * unit, 0 * unit, 7 * unit, 7 * unit);
        ctx.drawImage(eye, 0 * unit, (datalistlength - 7) * unit, 7 * unit, 7 * unit);
        ctx.draw();
},
    // 返回图片src
    imgSrc(num, type) {
