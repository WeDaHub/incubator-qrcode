    ...
    setImageData: function(imageData, imageWidth, imageHeight) {
        ...
        if (imageData.maxCol - imageData.minCol < 255 / 10) {
            throw new QRBase.QRError(
                'Image does not have enough contrast (this.image_data.min_col=' + imageData.minCol + ' this.image_data.max_col=' + imageData.maxCol + ')',
    ...
    extractData: function() {
        ...
        while (this.bitIdx < nBits - 4) {
            ...
            if (mode === QRBase.MODE.Terminator) {
                break;
            } else if (mode === QRBase.MODE.AlphaNumeric) { this.data += extractAlphanum(this, bytes); } else if (mode === QRBase.MODE.EightBit) { this.data += extract8bit(this, bytes); } else if (mode === QRBase.MODE.Numeric) { this.data += extractNumeric(this, bytes); } else {
